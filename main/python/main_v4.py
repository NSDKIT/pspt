...
def generate_combinations_all(res_list, M_list, yoc_list, bus_list, df_list, k1_values):
    combinations = []
...
def create_combinations(TargetOfNumber, k1_start, k1_end, delta_k1, df_list):
    res_list = [0]
    # res_list = [0.8, 0.5, 0.2]
    M_list = [8]
    yoc_list = [0.25]
    bus_list = range(11, 48)

    if k1_start == 250:
        k1_values = list(range(k1_start, k1_end, delta_k1))
    elif k1_start == 1:
        k1_values = [x / 100 for x in list(range(k1_start, k1_end, delta_k1))]

    combinations = generate_combinations_all(res_list, M_list, yoc_list, bus_list, df_list, k1_values)
    return combinations
...
def main(start_combination, combinations, sheet_name, start_row, start_col, PL_base, ContingencyCase, TargetOfNumber, folder_path, path_to_folder, condition_list_folder, script_path, excel_file_path, source_folder, source, destination_directory, original_file_sys, new_file_sys, new_directory_sys, new_zip_file_sys, original_file_condition, new_file_condition, new_directory_condition, new_zip_file_condition, Lp_i, Lq_i, df_list):
    ...
    print(len(remaining_combinations)*2/60) # 1ケース2分で計算 [時間]
    # remaining_combinations = combinations[start_index:]
    ...
    for combination in remaining_combinations:
        ...
        # Define mdl_base_name based on the current model
        mdl_base_name = f"M{m_value}-RES{res*100}%-YCbus{int(bus_value)}-{int(yco_value * 1000)}MVA-ΔP{int(k1_value)}MW"
...
if __name__ == "__main__":
    ...
    df_list = generate_combinations(11, 47, 10, TargetOfNumber)  # 引数は実際のものに置き換えてください
    ...
    for start_combination in combinations:
        start_combination = (0.8, 16, 0.25, 47, 0, 500) # (res, m_value, yco_value, bus_value, df, k1_value)
